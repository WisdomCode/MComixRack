...
####
    ...
    def add_metadata_to_bookmarks(self, bookmarks):
        """Adds Metadata from the Comicinfo, only if the pages are not already bookmarked. 
           Bookmarks is in this case a list of a 5er list, containing Bookmark name, page, path, 
           number of pages and archive type"""
        for bookmark in bookmarks:
            name = bookmark[0]
            page = int(bookmark[1])
            path = bookmark[2]
            numpages = int(bookmark[3])
            archive_type = bookmark[4]
            date_added = datetime.datetime.now()
