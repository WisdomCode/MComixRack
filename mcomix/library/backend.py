...
class _LibraryBackend(object):
    ...
    def get_book_by_path(self, path):
        ...
        cur = self.execute('''select id, name, path, pages, format,
                                     size, added, writer, summary, characters, genre, year, language
        ...
        try:
            ...
            if old is not None:
                cursor.execute('''update Book set
                    ...
                    (name, pages, format, size, mi[0], mi[1], mi[2], mi[3], mi[4], mi[5], path))
                book_id = old
            else:
                ...
                book = backend_types._Book(book_id, name, path, pages,
                        format, size, datetime.datetime.now().isoformat(), mi[0], mi[1], mi[2], mi[3], mi[4], mi[5])
