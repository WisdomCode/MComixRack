...
class _Book(_BackendObject):
    ...
    def __init__(self, id, name, path, pages, format, size, added, writer, summary, characters, genre, year, language):
    ...
    def get_books(self, filter_string=None):
        ...
        for collection in [ self ] + self.get_all_collections():
            sql = '''SELECT book.id, book.name, book.path, book.pages, book.format,
                            book.size, book.added, book.writer, book.summary, book.characters, book.genre, book.year, book.language
                     FROM book
            ...
            if filter_string:
                sql += ''' WHERE (book.name || book.writer || book.characters || book.genre || book.year)
                           LIKE '%' || ? || '%' '''
