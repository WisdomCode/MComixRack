...
class FileHandler(object):
    ...
    def open_file(self, path, start_page=0, keep_fileprovider=False):
        ...
        bookmark_backend.BookmarksStore.clear_bookmarks()
        metadata = archive_tools.get_archive_metadata(path)
        if metadata:
            bookmarkinfo = []
            for info in metadata[7]:
                bookmarkinfo.append([info[1], info[0], path, metadata[6], self.archive_type])
        bookmark_backend.BookmarksStore.add_metadata_to_bookmarks(bookmarkinfo[::-1])
