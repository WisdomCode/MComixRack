...
def get_archive_metadata(path):
    """Return a tuple (mime, num_pages, size) with Metadata info about the archive in the form of a ComicInfo.xml
    at <path>, or None if <path> doesn't point to a supported
    """
    cleanup = []
    ...
    files = archive.list_contents()
    try:
        metafile = files[[file.lower() for file in files].index("comicinfo.xml")]
    except ValueError:
        return None
    archive.extract(metafile, tmpdir)

    tree = ET.parse(os.path.join(tmpdir, metafile))
    metadata=[]

    metatags=["Writer", "Summary", "Characters", "Genre", "Year", "LanguageISO", "PageCount"]
    for tag in metatags:
        if tree.find(tag) is not None:
            metadata.append(tree.find(tag).text)
        else:
            metadata.append(None)

    chapterlist=[]
    for page in tree.find('Pages').getchildren():
        if "Bookmark" in page.attrib:
            chapterlist.append([int(page.attrib["Image"])+1, page.attrib["Bookmark"]])
        elif "Type" in page.attrib:
            chapterlist.append([int(page.attrib["Image"])+1, page.attrib["Type"]])
    metadata.append(chapterlist)
